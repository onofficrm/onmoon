...
<form name="fitemqalist" method="post" action="./chat_list_update.php" onsubmit="return fitemqalist_submit(this);" autocomplete="off">
    ...
    for ($i=0; $rowss=sql_fetch_array($result_is); $i++) {
     ...
     ?>
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td class="td_datetime td_name sv_use" nowrap>
                        <div><?php echo $mb_nick2; ?></div>
                    </td>
                    <td class="td_left">
    ...
    <div class="btn_fixed_top">
        ...
        <input type="button" value="확인" class="btn_submit btn" id="btn_submit_form">
    </div>
</form>
    ...
    function fitemqalist_submit(f) {
        ...
        if (document.pressed === "선택삭제") {
            if (typeof window.rb_confirm === 'function') {
                rb_confirm("선택한 자료를 정말 삭제하시겠습니까?")
                    .then(function(ok) {
                        if (ok) goSubmit("선택삭제");
                    });
                return false;
        ...
        if (document.pressed === "전체삭제") {
            if (typeof window.rb_confirm === 'function') {
                rb_confirm("전체 데이터를 삭제하시겠습니까?\n삭제된 데이터는 복구되지 않습니다.")
                    .then(function(ok) {
                        if (ok) goSubmit("전체삭제");
                    });
                return false;
            }
            return confirm("전체 데이터를 삭제하시겠습니까?\n삭제된 데이터는 복구되지 않습니다.");
        }
