    ...
    <div id="bo_v_con">
        ...
        for ($i=1; $i<=10; $i++) {
            ...
            if (strpos($spec,'|')!==false){
                list($type,$optstr)=array_map('trim',explode('|',$spec,2));
                $opts=array_values(array_filter(array_map('trim',explode(',',$optstr))));
            } else {
                $type=trim($spec);
            }

            $name = "wr_{$i}";
            // 보기페이지는 $view 배열에 저장된 값을 사용
            $cur = (string)($view[$name] ?? '');
            $curSet = $cur!=='' ? array_map('trim', explode(',', $cur)) : [];

            $fields[] = compact('name','subj','type','opts','cur','curSet');
        }
        ?>

        <?php if (!empty($fields)) { ?>
        <div class="rb-extra-grid rb-extra-grid-view">
          <?php foreach ($fields as $f):
                ...
                // 표시 문자열 만들기 (checkbox는 콤마로)
                if ($type === 'checkbox') {
                    $parts = [];
                    foreach ($curSet as $it) {
                        $parts[] = htmlspecialchars($it, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                    }
                    $display = implode(', ', $parts);
                } else {
                    $display = htmlspecialchars($cur, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                }
          ...
          ?>
            ...
            <div class="rb-input"><?php echo ($display !== '' ? $display : '-'); ?></div>
          <?php endforeach; ?>
        </div>
        <br><br>
        <?php } ?>
